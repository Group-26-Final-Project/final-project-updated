    ...
    <div class="flex flex-col items-center justify-center min-h-screen bg-[#2F313D]">
      <div class="px-12 py-8 mt-4 text-left bg-white shadow-lg w-[40vw]">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div class="flex items-baseline justify-center gap-x-5">
            <button class="px-6 py-2 mt-4 text-white bg-[#00D05A] rounded-lg hover:bg-green-900">
              Submit
            </button>
            <button class="px-6 py-2 mt-4 text-white  rounded-lg bg-gray-400">
              Cancel
            </button>
          </div>
        </form>
        {modalOn && (
          <Modal setModalOn={setModalOn} setInfo={setInfo} info={info} />
        )}
      </div>
    </div>
  );
